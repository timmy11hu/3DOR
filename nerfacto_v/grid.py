...
class DenseGrid(nn.Module):
    ...
    # def smooth_visibility_grid(self):
    #     def median_filter_3d(grid, kernel_size):
    #         """Applies a 3D median filter to the grid."""
    #         padding = kernel_size // 2
    #         grid_padded = F.pad(grid, (padding, padding, padding, padding, padding, padding), mode='reflect')
    #         patches = grid_padded.unfold(2, kernel_size, 1).unfold(3, kernel_size, 1).unfold(4, kernel_size, 1)
    #         patches = patches.contiguous().view(patches.size()[:5] + (-1,)).median(dim=-1)[0]
    #         return patches
    #     self.grid = nn.Parameter(median_filter_3d(self.grid, kernel_size=5))
