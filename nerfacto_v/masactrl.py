...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def __init__(self, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50, model_type="SD", lambda_attn=0.1):
        """
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        # lambda_attn = 0.2
        # out_u = (lambda_attn * self.attn_batch(qu, ku, vu, sim, attnu, is_cross, place_in_unet, num_heads, **kwargs) +
        #          (1.-lambda_attn) * self.attn_batch(qu, ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs))
        # out_c = (lambda_attn * self.attn_batch(qc, kc, vc, sim, attnc, is_cross, place_in_unet, num_heads, **kwargs) +
        #          (1.-lambda_attn) * self.attn_batch(qc, kc[:num_heads], vc[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs))
        # print(out_u.shape, out_c.shape)

        lambda_attn = self.lambda_attn
        out_u_0 = self.attn_batch(qu[:num_heads], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        if lambda_attn > 0.:
            out_u_1 = (lambda_attn * self.attn_batch(qu[num_heads:], ku[num_heads:], vu[num_heads:], sim[num_heads:], attnu, is_cross, place_in_unet, num_heads, **kwargs) +
                 (1.-lambda_attn) * self.attn_batch(qu[num_heads:], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs))
        elif lambda_attn == 0.:
            out_u_1 = self.attn_batch(qu[num_heads:], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        elif lambda_attn == 1.:
            out_u_1 = self.attn_batch(qu[num_heads:], ku[num_heads:], vu[num_heads:], sim[num_heads:], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        out_u = torch.cat([out_u_0, out_u_1], dim=0)

        out_c_0 = self.attn_batch(qc[:num_heads], kc[:num_heads], vc[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        if lambda_attn > 0.:
            out_c_1 = (lambda_attn * self.attn_batch(qc[num_heads:], kc[num_heads:], vc[num_heads:], sim[num_heads:], attnc, is_cross, place_in_unet, num_heads, **kwargs) +
                       (1. - lambda_attn) * self.attn_batch(qu[num_heads:], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs))
        elif lambda_attn == 0.:
            out_c_1 = self.attn_batch(qu[num_heads:], ku[:num_heads], vu[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        elif lambda_attn == 1.:
            out_c_1 = self.attn_batch(qc[num_heads:], kc[num_heads:], vc[num_heads:], sim[num_heads:], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        out_c = torch.cat([out_c_0, out_c_1], dim=0)
        # print(out_u.shape, out_c.shape)
