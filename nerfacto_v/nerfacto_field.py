...
class NerfField(Field):
    ...
    def __init__(
        ...
        self.vis_grid = HashEncoding(
            num_levels=1,
            min_res=self.field.max_res,
            max_res=self.field.max_res,
            log2_hashmap_size=self.field.log2_hashmap_size,
            features_per_level=1,
            hash_init_scale=0.,  # 0.001
    ...
    def forward(self, ray_samples: RaySamples, compute_normals: bool = False) -> Dict[FieldHeadNames, Tensor]:
        ...
        if compute_normals:
            with torch.enable_grad():
                density, density_embedding, visibility = self.get_density(ray_samples)
        else:
            density, density_embedding, visibility = self.get_density(ray_samples)
        ...
        print("field: vis shape: ", visibility.shape)
        field_outputs["visibility"] = visibility.mean(-2) # ray dimension, mean coule be replaced by volume rendering
    ...
    def get_visibility(self, ray_samples: RaySamples) -> Tuple[Tensor, Tensor]:
